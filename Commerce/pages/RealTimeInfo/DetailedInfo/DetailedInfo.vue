		...
		<view v-if="dataForm">
			...
			<view class="comment">
				...
				<view class="comment-list">
					<!-- 某个评论 -->
					<view class="comment-item" @click="showReply(item)" v-for="(item,index) in commentForm" :key="index">
						...
						<view class="comment-text">
							...
							<view class="comment-item-tile">
								<view class="comment-item-reply">
									<span class="comment-item-reply-text">{{ item.replyNum==0?'':item.replyNum }}回复</span>
									<span class="iconfont comment-item-to">&#xe623;</span>
								</view>
								<view class="comment-item-time">{{ formatMsgTime(item.createTime) }}</view>
								<span class="iconfont comment-item-delete"  v-if="judgeDelete(item.openId)" v-on:click.stop="deleteComment(item.id)">&#xe608;</span>
							</view>
						</view>
					</view>
				</view>
			...
			<view class="reply-page" v-if="actComment">
				...
				<view class="comment-item" style="border-bottom: 1rpx solid #e8e8e8;">
					...
					<view class="comment-text" style="border: none">
						...
						<view class="comment-item-tile"><view class="comment-item-time">{{ formatMsgTime(actComment.createTime) }}</view></view>
					</view>
				</view>
				<view class="comment-list">
					...
					<view class="comment-item" v-for="(item,index) in actCommentReply" :key="index">
						...
						<view class="comment-text">
							...
							<view class="comment-item-tile">
								<view class="comment-item-time">{{ formatMsgTime(item.createTime) }}</view>
								<span class="iconfont comment-item-delete" v-if="judgeDelete(item.openId)" v-on:click.stop="deleteComment2(item.id)">&#xe608;</span>
							</view>
						</view>
					</view>
				</view>
		...
		pubComment2(content) {
			...
			this.$api.postReply(getAPI).then((res) => {
				...
				if(res.success){
					...
					let getAPI2 = {commentId: this.actComment.id}
